    ...
    if(task) {
      ...
      TaskList._registeredTasks[task.type](session, options, function(err) {
        if(err) {
          ...
          var durationText = '';
          if (options && options.showDuration) {
            var endTime = new Date();
            durationText = ' - ' + (endTime.getTime() - startTime.getTime()) + 'ms';
          }
          ...
          self.log('log', util.format('[%s] '.magenta+'- %s: SUCCESS'.green + durationText, session._host, task.id));
